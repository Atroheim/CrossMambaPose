...
class NoCrossBlock(nn.Module):
    ...
    def forward(self, x_Rng, x_mD):
        x_Rng = x_Rng + self.mamba_Rng(self.norm_Rng(x_Rng))
        x_mD  = x_mD  + self.mamba_mD(self.norm_mD(x_mD))
...
class main_Net(nn.Module):
    def __init__(self, args):
        ...
        self.layers=nn.ModuleList([NoCrossBlock(self.d_model) for _ in range(self.num_layers)])
