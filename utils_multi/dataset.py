...
class RadarDataset_Keypoint(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1. 删掉之前加的 import h5py
        # 2. 修改这一行（注意后缀是 .npy，且不需要 ['reconstruction']）
        keypoint_path_real = keypoint_path + '/output_3D/keypoints.npy'
        keypoint_3D = np.load(keypoint_path_real, allow_pickle=True)
